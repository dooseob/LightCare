...
DELETE FROM member
WHERE member_id IN (10);
use carelink;
ALTER TABLE member ADD COLUMN detail_address
    TEXT COMMENT '상세 주소' AFTER address;
SHOW CREATE TABLE member;
DESCRIBE member;


-- 1. 회원 테이블에 프로필 이미지 alt 텍스트
ALTER TABLE member
    ADD COLUMN profile_image_alt_text VARCHAR(255)
        COMMENT '프로필 이미지 alt 텍스트 (SEO
  최적화용)'
        AFTER profile_image;

-- 2. 시설 테이블에 시설 이미지 alt 텍스트 컬럼
ALTER TABLE facility
    ADD COLUMN facility_image_alt_text VARCHAR(255)
        COMMENT '시설 이미지 alt 텍스트 (SEO 최적화용)'
        AFTER facility_image;
...
WHERE TABLE_SCHEMA = 'carelink'
  ...
  AND COLUMN_NAME = 'profile_image_alt_text';
...
WHERE TABLE_SCHEMA = 'carelink'
  ...
  AND COLUMN_NAME = 'facility_image_alt_text';


-- 1. 시설 이미지 테이블 생성 (다중 이미지 저장용)
CREATE TABLE facility_images (
                                 ...
                                 INDEX idx_image_order (facility_id, image_order)
);

-- 2. 기존 facility 테이블의 이미지 데이터를 새 테이블로 마이그레이션
INSERT INTO facility_images (facility_id, image_path, image_alt_text, is_main_image, image_order)
SELECT
    facility_id,
    facility_image,
    COALESCE(facility_image_alt_text, CONCAT(facility_name, ' 시설 이미지')),
    TRUE,  -- 기존 이미지는 메인 이미지로 설정
    0      -- 첫 번째 순서
FROM facility
WHERE facility_image IS NOT NULL AND facility_image != '';

-- 3. facility 테이블에 다중 이미지 지원을 위한 컬럼 추가 (선택사항)
-- 기존 컬럼은 호환성을 위해 유지하되, 새로운 컬럼 추가
ALTER TABLE facility
    ADD COLUMN image_count INT DEFAULT 0,
    ADD COLUMN main_image_path VARCHAR(500);

-- 4. 기존 데이터의 image_count와 main_image_path 업데이트
UPDATE facility f
SET
    image_count = (
        SELECT COUNT(*)
...
SELECT
    ...
    f.image_count,
    f.main_image_path,
    GROUP_CONCAT(fi.image_path ORDER BY fi.image_order) as all_images
FROM facility f
         LEFT JOIN facility_images fi ON f.facility_id = fi.facility_id
GROUP BY f.facility_id, f.facility_name, f.image_count, f.main_image_path;

-- 참고: 기존 facility_image, facility_image_alt_text 컬럼은
-- 하위 호환성을 위해 삭제하지 않습니다.
-- 필요시 나중에 DROP할 수 있습니다:
-- ALTER TABLE facility DROP COLUMN facility_image;
-- ALTER TABLE facility DROP COLUMN facility_image_alt_text;