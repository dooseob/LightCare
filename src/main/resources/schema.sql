...
INSERT INTO member (
    ...
    is_deleted
      ...
      ('admin', '$2a$10$N.DS.GzSOBaa/SZbQ5xt/eHsIOBYjw/08oBN2aQWTk0X.8Ej8JIRG', '관리자', 'admin@carelink.com', '02-1234-5678', 'ADMIN', '서울시 종로구 세종대로 1', TRUE, 0, NOW(), NOW(), FALSE),
      ('user01', '$2a$10$7KQO7MNnQ9XwKMM4jfI.kOV5XxJm1ZLm3qhIHa/Q.8QkXvKsMAJLq', '김철수', 'kim.cs@example.com', '010-1111-2222', 'USER', '서울시 강남구 테헤란로 123', TRUE, 0, NOW(), NOW(), FALSE),
      ('user02', '$2a$10$7KQO7MNnQ9XwKMM4jfI.kOV5XxJm1ZLm3qhIHa/Q.8QkXvKsMAJLq', '이영희', 'lee.yh@example.com', '010-2222-3333', 'USER', '부산시 해운대구 센텀중앙로 79', TRUE, 0, NOW(), NOW(), FALSE),
      ('user03', '$2a$10$7KQO7MNnQ9XwKMM4jfI.kOV5XxJm1ZLm3qhIHa/Q.8QkXvKsMAJLq', '박민수', 'park.ms@example.com', '010-3333-4444', 'USER', '대구시 중구 동성로 1', TRUE, 0, NOW(), NOW(), FALSE),
      ('user04', '$2a$10$7KQO7MNnQ9XwKMM4jfI.kOV5XxJm1ZLm3qhIHa/Q.8QkXvKsMAJLq', '최지은', 'choi.je@example.com', '010-4444-5555', 'USER', '인천시 남동구 구월동 1234', TRUE, 0, NOW(), NOW(), FALSE),
      ('facility01', '$2a$10$E6/mbtPf3YFqx.F.2Fg5/OXhZqL.a4QfgWEFTnmGr8.gFYJF4Qh3K', '서울요양원장', 'seoul.admin@example.com', '02-5555-6666', 'FACILITY', '서울시 서초구 서초대로 123', TRUE, 0, NOW(), NOW(), FALSE),
      ('facility02', '$2a$10$E6/mbtPf3YFqx.F.2Fg5/OXhZqL.a4QfgWEFTnmGr8.gFYJF4Qh3K', '부산실버타운장', 'busan.admin@example.com', '051-7777-8888', 'FACILITY', '부산시 부산진구 서면로 456', TRUE, 0, NOW(), NOW(), FALSE);
-- 현재 상태 확인
SELECT user_id, LEFT(password, 20) as hash_preview, LENGTH(password) as len FROM member;

-- 검증된 BCrypt 해시로 업데이트 (온라인 생성기에서 검증됨)
-- admin123
UPDATE member SET password = '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2uheWG/tn4G' WHERE user_id = 'admin';

-- user123
UPDATE member SET password = '$2a$10$DowJoTG8PmBPVJZzGaZ6XO3WE0e6WPgDbBXLu1F/R1GWI5/DKv3W2' WHERE user_id IN ('user01', 'user02', 'user03', 'user04');

-- facility123
UPDATE member SET password = '$2a$10$ZDgKz6VJIyAGzfSF2i9XBuG8TKs2r5yy/gXlHMaEqRr.ygk1uFXdO' WHERE user_id IN ('facility01', 'facility02');

-- 계정 상태 정상화
UPDATE member SET login_fail_count = 0, is_active = TRUE, is_deleted = FALSE;

-- 결과 확인
SELECT user_id, LEFT(password, 30) as new_hash, LENGTH(password), is_active, login_fail_count FROM member;

SELECT '비밀번호 업데이트 완료!' as status;

SELECT 'BCrypt 비밀번호 업데이트 완료!' as status;
...
-- 결과 확인
SELECT user_id, LEFT(password, 20) as password_hash, role FROM member;

SELECT 'BCrypt 비밀번호가 적용된 회원 데이터 삽입 완료!' as status;
-- 시설 데이터
